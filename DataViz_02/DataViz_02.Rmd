...
The first priority in DataViz Makeoever is to retain the original design as much as possible, and improve on the current design. To that end, the proposed design considers the following:
  ...
  2. To that end, a chloropleth map of Singapore will be used in place of the bar charts. Although this replaces the original choice of chart tyoe, and should not usually be encouraged, it is crucial for this particular dataset that we use a map to show the geographical dimension of the data.
...
Tableau Desktop was used for the visualisations. To reproduce this visualisation makeover, detailed steps can be found at the ["Data Prep Log"](https://mviz.netlify.app/dataviz_01/dataprep/dataprep_dv1) section.
