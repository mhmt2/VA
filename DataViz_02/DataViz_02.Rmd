...
knitr::include_graphics("sketch3.jpg")
...
2a. **Layout**: Used Story to create tabs within the page layout where the map views, and adjacency matrices were put in separate tabs within the same page so that we can view each visualisation more clearly, yet still give viewers the sense that it's still within  the same visualisation. 

2b. The option of trying to fit both weekday and weekend matrices on the same page was attempted and rejected as they did not allow viewers to see both visuals at a glance without scrolling. Shrinking the 2 grids to allow for a single screen view severely compromised the interpretability and usefulness of the matrices and so the final decision was made to use tabs within the dashboard as a least worst option. 
...
### Main tab (Map view)
```{r}
knitr::include_graphics("dataviz7.png")
```

### Tab 2 (Weekday OD Matrix)
```{r}
knitr::include_graphics("dataviz5.png")
```

### Tab 3 (Weekend OD Matrix)
```{r}
knitr::include_graphics("dataviz6.png")
```
